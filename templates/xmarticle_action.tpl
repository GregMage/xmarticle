<div class="xmarticle">
	...
	<{if $form|default:false}>
		<nav aria-label="breadcrumb">
		  <ol class="breadcrumb">
			...
			<li class="breadcrumb-item"><a href="action.php?op=add"><{$smarty.const._MA_XMARTICLE_SELECTCATEGORY}></a></li>
			<li class="breadcrumb-item active" aria-current="page"><{$smarty.const._MA_XMARTICLE_ADD}></li>
		  </ol>
		</nav>
        <div class="xmform">
            <{$form}>
    ...
    <{if $categories|default:0 > 0}>
		<nav aria-label="breadcrumb">
		  <ol class="breadcrumb">
			<li class="breadcrumb-item"><a href="index.php"><{$index_module}></a></li>
			<li class="breadcrumb-item active" aria-current="page"><{$smarty.const._MA_XMARTICLE_SELECTCATEGORY}></li>
		  </ol>
		</nav>
			...
			<div class="col-6 col-sm-4 col-md-3 p-2">
				<div class="card" <{if $category.color != false}>style="border-color : <{$category.color}>;"<{/if}>>
					<div class="card-header text-center" <{if $category.color != false}>style="background-color : <{$category.color}>;"<{/if}>>
						<a class="text-decoration-none" title="<{$category.name}>" href="action.php?op=loadarticle&category_id=<{$category.id}>">
							<{$category.name}>
						</a>
					</div>
						<div class="card-body h-md-550 text-center">
							<div class="row" style="height: 150px;">
								<div class="col-12 h-75">
									<{if $category.logo != ''}>
									<a title="<{$category.name}>" href="action.php?op=loadarticle&category_id=<{$category.id}>">
